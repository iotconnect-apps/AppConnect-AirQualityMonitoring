    ...
    if (this.facilityForm.status === "VALID") {
      ...
      this.facilityForm.get('parentEntityGuid').setValue(currentUser.userDetail.entityGuid);
        ...
        this._service.addFacility(this.facilityForm.value).subscribe(response => {
          ...
          if (response.isSuccess === true) {
            if (this.isEdit) {
              this._notificationService.add(new Notification('success', "Facility has been updated successfully."));
            } else {
              this._notificationService.add(new Notification('success', "Facility has been added successfully."));
            }
