...
BEGIN
    ...
    BEGIN TRY
		...
		IF OBJECT_ID('tempdb..#temp_AvgAttributeValue') IS NOT NULL DROP TABLE #temp_AvgAttributeValue
		...
		, CTE_attribute
		AS 
		(	SELECT [attribute],E.[parentEntityGuid],AVG([avg]) AS [value] 
			...
			WHERE E.[companyGuid] = @companyGuid AND T.[attribute] IN ('temp','humidity')
			GROUP BY [attribute],E.[parentEntityGuid]
		)

		UPDATE L
		SET [totalDevices]	= ISNULL(CC.[totalCount],0) + ISNULL(C.[totalCount],0) 
			, [totalSubEntities]	= ISNULL(CW.[totalCount],0)
			, [AvgTemperature]		= ISNULL(attTemp.[value],0)
			, [AvgHumidity]			= ISNULL(attHumidity.[value],0)
		FROM #temp_Entity L
		...
		LEFT JOIN CTE_attribute attTemp	ON L.[guid] = attTemp.[parentEntityGuid] AND attTemp.[attribute] = 'temp'
		LEFT JOIN CTE_attribute attHumidity	ON L.[guid] = attHumidity.[parentEntityGuid] AND attHumidity.[attribute] = 'humidity'

		-- #temp_AvgAttributeValue --dbo.[KitTypeAttribute]
