...
namespace iot.solution.service.Data
{
    public class LookupService : ILookupService
    {
        ...
        public List<Entity.LookupItem> ZoneLookup(Guid facilityId, string type)
        {
            List<Entity.LookupItem> result = new List<Entity.LookupItem>();
            var template = _entityRepository.FindBy(t => t.Guid == facilityId).FirstOrDefault();
            if (template != null)
            {
                result = (from t in _entityRepository.GetAll()
                          where t.CompanyGuid.Equals(SolutionConfiguration.CompanyId) && t.ParentEntityGuid.Equals(facilityId) && (string.IsNullOrWhiteSpace(type) || t.Type.Equals(type))  && !t.IsDeleted
                          select new Entity.LookupItem()
                          {
                              Text = t.Name,
                              Value = t.Guid.ToString()
